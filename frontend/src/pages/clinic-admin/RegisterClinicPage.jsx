    ...
    <section className="min-h-screen flex items-center justify-center bg-background py-8">
      <div className="bg-white sm:p-8 rounded-2xl sm:shadow-lg w-full max-w-2xl">
        ...
        <div className="space-y-4">
          {/* Clinic Name */}
          <div>
            <label className="label-required block text-sm text-gray-700 mb-2">Clinic Name</label>
            <input
              type="text"
              name="clinicName"
          ...
          {/* Address */}
          <div>
            <label className="label-required block text-sm text-gray-700 mb-2">Street Address</label>
            <input
              type="text"
              name="address"
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="label-required block text-sm text-gray-700 mb-2">City</label>
              <input
                type="text"
                name="city"
                value={formData.city}
            ...
            <div>
              <label className="label-required block text-sm text-gray-700 mb-2">State</label>
              <input
                type="text"
                name="state"
            ...
            <div>
              <label className="label-required block text-sm text-gray-700 mb-2">ZIP</label>
              <input
                type="text"
                name="zipCode"
          ...
          {/* Operating Hours */}
          <div>
            <label className="label-required block text-sm text-gray-700 mb-2">Operating Hours</label>
            <input
              type="text"
              name="hours"
          ...
          {/* Clinic Picture */}
          <div>
            <label className="label-required block text-sm text-gray-700 mb-2">Clinic Picture</label>
            {previews.picture ? (
              <div className="border border-gray-300 rounded-2xl p-4">
                <img src={previews.picture} alt="Clinic" className="w-full h-48 object-cover rounded-2xl mb-3" />
                <button onClick={() => removeFile('picture')} className="w-full py-2 text-sm text-red-600 border border-red-600 rounded-2xl hover:bg-red-50 transition">
                  Remove
                </button>
              </div>
            ) : (
              <div className="border border-gray-300 rounded-2xl p-6 text-center">
                <input type="file" accept="image/*" onChange={(e) => handleFileChange(e, 'picture')} className="hidden" id="picture" />
                <label htmlFor="picture" className="cursor-pointer">
                  <p className="text-sm text-gray-600 mb-2">Upload clinic picture</p>
          ...
          {/* License */}
          <div>
            <label className="label-required block text-sm text-gray-700 mb-2">License/Permit</label>
            {previews.license ? (
              <div className="border border-gray-300 rounded-2xl p-4">
                <img src={previews.license} alt="License" className="w-full h-48 object-cover rounded-2xl mb-3" />
                <button onClick={() => removeFile('license')} className="w-full py-2 text-sm text-red-600 border border-red-600 rounded-2xl hover:bg-red-50 transition">
                  Remove
                </button>
              </div>
            ) : (
